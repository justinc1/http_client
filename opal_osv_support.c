...
char* opal_argv_join(char *argv[], char sep) {
    int len = 1, remain_len, delta_len;
    char** prm;
    for (prm = argv; prm != NULL && *prm != NULL; prm++) {
        len += strlen(*prm) + 1;
    }
    char *out2, *out;
    out2 = out = (char*)malloc(len);
    remain_len = len;
    for (prm = argv; prm != NULL && *prm != NULL; prm++) {
        delta_len = snprintf(out, remain_len, "%s%c", *prm, sep);
        out += delta_len;
        remain_len -= delta_len;
    }
    out[0] = 0x00; // the last sep is still in out.
    return out2;
}
